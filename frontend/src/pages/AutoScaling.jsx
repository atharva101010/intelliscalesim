  ...
  const handleStartAutoScaler = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        alert('❌ Failed to start: ' + response.message);
      }
    } catch (error) {
      console.error('Error starting auto-scaler:', error);
      setAutoScalerActive(false); // Revert
      alert('❌ Failed to start auto-scaler');
    } finally {
      setIsStarting(false);
    }
  };
  ...
  const handleStopAutoScaler = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        alert('❌ Failed to stop: ' + response.message);
      }
    } catch (error) {
      console.error('Error stopping auto-scaler:', error);
      setAutoScalerActive(true); // Revert
      alert('❌ Failed to stop auto-scaler');
    } finally {
      setIsStopping(false);
    }
  };
    ...
    <div style={{ padding: '30px', minHeight: '100vh', background: '#f3f4f6' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', padding: '20px', borderRadius: '12px', marginBottom: '24px', color: 'white', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
        <div style={{ display: 'flex', alignItems: 'start', gap: '12px' }}>
          ...
          <div>
            <h3 style={{ fontSize: '18px', fontWeight: 'bold', marginBottom: '8px' }}>📚 Real-Time Container Auto-Scaling</h3>
            <p style={{ fontSize: '14px', lineHeight: '1.6', opacity: 0.95 }}>
              ...
              {!dockerStatus.running && <span style={{ color: '#fef3c7' }}> ⚠️ Docker is not connected.</span>}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Header */}
      <div style={{ background: '#FFFBF5', padding: '28px', borderRadius: '16px', marginBottom: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px', background: dockerStatus.running ? '#dcfce7' : '#fee2e2', borderRadius: '20px' }}>
                <span style={{ fontSize: '14px', color: dockerStatus.running ? '#065f46' : '#991b1b', fontWeight: '600' }}>
                  🐳 Docker: {dockerStatus.running ? 'Connected' : 'Disconnected'}
                </span>
              </div>
            </div>
          </div>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px', background: '#FFFBF5', borderRadius: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
            {/* CPU Usage Card */}
            <div style={{ background: '#FFFBF5', padding: '28px', borderRadius: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
              ...
              <div style={{ marginTop: '16px', display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: '600' }}>
                <span style={{ color: '#f59e0b' }}>↓ {config.cpuScaleDown}%</span>
                <span style={{ color: '#ef4444' }}>↑ {config.cpuScaleUp}%</span>
              </div>
            </div>
            ...
            <div style={{ background: '#FFFBF5', padding: '28px', borderRadius: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
              ...
              <div style={{ marginTop: '16px', display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: '600' }}>
                <span style={{ color: '#f59e0b' }}>↓ {config.memScaleDown}%</span>
                <span style={{ color: '#ef4444' }}>↑ {config.memScaleUp}%</span>
              </div>
            </div>
      ...
      <div style={{ background: '#FFFBF5', padding: '28px', borderRadius: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', marginBottom: '24px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '15px', fontWeight: 'bold', color: '#1f2937', marginBottom: '4px' }}>
                        {event.action}
                        {event.from !== event.to && (
                          <span style={{ marginLeft: '10px', padding: '4px 10px', background: '#FFFBF5', color: '#6b7280', fontWeight: '600', borderRadius: '6px', fontSize: '13px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {container.metrics && container.metrics.running && (
                      <div style={{ fontSize: '13px', color: '#6b7280', display: 'flex', gap: '16px', marginTop: '8px' }}>
                        <span>💻 CPU: <strong style={{ color: getCpuColor(container.metrics.cpu) }}>{container.metrics.cpu}%</strong></span>
                        <span>🧠 Memory: <strong style={{ color: getMemoryColor(container.metrics.memory) }}>{container.metrics.memory}%</strong></span>
                        <span>📊 {container.metrics.memoryUsage}</span>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div style={{ background: '#FFFBF5', padding: '24px', borderRadius: '12px', marginBottom: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
        <h3 style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '16px' }}>⚙️ Configuration</h3>
